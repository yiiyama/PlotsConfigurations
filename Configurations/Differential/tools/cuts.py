...
def addcr(name, binning, cutsMap, slist):
    ...
    for bname in binning:
        cuts[name]['categories'].append('%s_top' % bname)
        if bname != binning[0]:
            topcat.append('(%s)' % cutsMap[bname])
    
    dycat = []
    for bname in binning:
        cuts[name]['categories'].append('%s_DY' % bname)
        if bname != binning[0]:
            dycat.append('(%s)' % cutsMap[bname])
    
    wwcat = []
    for bname in binning:
        cuts[name]['categories'].append('%s_WW' % bname)
        if bname != binning[0]:
        ...
        for pt2cat, _ in pt2cats:
            for flavcat, _ in flavcats:
                for chargecat, _ in chargecats:
                    cuts[name]['categories'].append('%s_cat%s%s%s' % (bname, pt2cat, flavcat, chargecat))
